...
- Voice initial briefing prepares separate dispatch and page proposals without execution; one explicit utterance can consume exactly the named pre-read tokens. Rollback retains a separate commit/base readback.
- Devin requests bounded structured output for input needed, failure reason, actual changes and validation. Provider-attributed details are credential-redacted before audit/Telegram; omissions are stated honestly. They never bypass the GitHub gate.
...
Final verification on 2026-09-18: **42 tests passed** on a fresh Python 3.12.12 environment; Ruff and compilation passed. Run `.venv/bin/python -m pytest -q` to reproduce. Test fakes are under `tests/` only. No integration test result implies provider credentials, latency, or production setup are valid. The runtime dependency constraints are pinned in requirements.lock. Installed FastAPI/Starlette emits two test-client deprecation warnings with httpx; application tests pass. Tests and constrained editable installation were verified on macOS with Python 3.14 and a fresh Python 3.12.12 environment. Docker/Linux container and Railway execution remain unverified (Docker engine unavailable).

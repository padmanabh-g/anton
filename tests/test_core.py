...
from anton.store import Store, Conflict

@pytest.fixture
def db(tmp_path):
    return Store(str(tmp_path/'anton.db'), Settings(run_id='run-1', deployed_sha='a'*40, seed_regression_sha='a'*40, known_good_sha='b'*40))

def alert(db, **kw):
    data=dict(event_id='e1',run_id='run-1',monitor_id='m1',group='demo',occurrence='o1',status='alert',deployed_sha='a'*40,error_count=5)
...
def test_approvals_bind_actor_targets_and_one_strategy(db):
    ...
    with pytest.raises(Conflict): db.approve(p['token'],'99','telegram')
    with concurrent.futures.ThreadPoolExecutor() as pool:
        results=list(pool.map(lambda _:db.approve(p['token'],'42','telegram'),range(8)))
    assert len({r['id'] for r in results})==1
    with pytest.raises(Conflict): db.propose(i['id'],'rollback','42','telegram')

def test_expired_approval_reset_and_late_alert(db):
    i=alert(db); p=db.propose(i['id'],'page_team','42','telegram')
    with db.tx() as c: c.execute('UPDATE approvals SET expires=0')
    with pytest.raises(Conflict):db.approve(p['token'],'42','telegram')
    p=db.propose(i['id'],'dispatch_fix','42','telegram')
...
def test_recovery_new_occurrence_and_append_only(db):
    ...
    with pytest.raises(sqlite3.DatabaseError):
        with db.tx() as c:c.execute('DELETE FROM events')

def test_expired_lease_becomes_unknown_never_requeued(db):
    i=alert(db); a=db.claim()
    with db.tx() as c:c.execute('UPDATE actions SET lease_until=0 WHERE id=?',(a['id'],))
    db.recover()
...
def test_voice_requires_server_conversation_binding(db):
    i=alert(db)
    with pytest.raises(Conflict):db.voice_context('invented','run-1')
    db.bind_call(i['id'],'conversation','call','initial')
    assert db.voice_context('conversation','run-1')['id']==i['id']
    db.reset('run-2','operator')
    with pytest.raises(Conflict):db.voice_context('conversation','run-1')

def test_run_reuse_without_incidents_is_rejected(db):
    db.reset('run-2','operator')
    with pytest.raises(Conflict):db.reset('run-1','operator')
